...
Program starts
    ↓
Does user file exist?
    ↓ YES                                    ↓ NO

Load user data                            start the user password and username taken
ask for the username and password         Collect all user data
Skip setup questions                      Save to user file then reload and satrt the programe again
    ↓                        ↓
Ask daily questions      Ask daily questions
- weight today           - weight today
- sleep hours            - sleep hours
- calories eaten         - calories eaten
    ↓
Save daily entry with today's date b b                       b
    ↓
Show summary , visual ones
"""
import os
import sqlite3 
import datetime
...
class User ():
    ...
    def insert_personal_data(self):
        ...
        cu.execute("INSERT INTO personal_information VALUES (:First, :Last, :Gender, :Birth_date,:Password , :Height)",
                   {'First': first, 'Last': last, 'Gender': gender, 'Birth_date': birth_date, 'Password':pwd , 'Height': height})
        con.commit()
        con.close()


    def chek_the_password(self):
        con = sqlite3.connect('user_data.db')
        cu = con.cursor()
        cu. execute ("SELECT Password FROM personal_file")
        stored_Password = cu.fetchone()
        while True:
            inpt_pass = input("what is the password ")
            if bcrypt.checkpw(inpt_pass.encode("utf-8"), stored_Password):
